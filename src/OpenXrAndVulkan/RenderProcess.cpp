...
: context(context), commandPool(commandPool), descriptorPool(descriptorPool), descriptorSetLayout(descriptorSetLayout)
{
    //UpdateRenderProcess();
}
...
void RenderProcess::AllocateDescriptorSetsUniformBuffers(size_t maxGameObjectCount, size_t maxMaterialsCount)
{
    // [tdbe] TODO: create a per-material buffer for e.g. brdf, colors, textures etc
    
    dynamicVertexUniformData.resize(maxGameObjectCount);
    dynamicFragmentUniformData.resize(maxGameObjectCount);

    // [tdbe] Note: the uniform data is in the header file

    const VkDevice device = context->getVkDevice();
    {
        // [tdbe] it would be very weird to try to allocate more than once!
    ...
    {
        util::DebugError("[RenderProcess] can't vkAllocateCommandBuffers!");
        valid = false;
        return;
    }
    ...
    {
        util::DebugError("[RenderProcess] can't setDebugObjectName!");
        valid = false;
        return;
    }
#endif
    ...
    {
        util::DebugError("[RenderProcess] can't vkCreateSemaphore drawable!");
        valid = false;
        return;
    }
    ...
    {
        util::DebugError("[RenderProcess] can't vkCreateSemaphore presentable!");
        valid = false;
        return;
    }
    ...
    {
        util::DebugError("[RenderProcess] can't vkCreateFencevkCreateFence!");
        valid = false;
        return;
    }
    ...
    descriptorBufferInfos.at(3u).offset =
        descriptorBufferInfos.at(2u).offset +
        util::align(descriptorBufferInfos.at(2u).range, uniformBufferOffsetAlignment) *
            static_cast<VkDeviceSize>(maxGameObjectCount); // [tdbe] notice the dynamic object count here. We add the size of the previous range above
    ...
    const VkDeviceSize uniformBufferSize =
        descriptorBufferInfos.at(3u).offset +
        util::align(descriptorBufferInfos.at(3u).range, uniformBufferOffsetAlignment) *
            static_cast<VkDeviceSize>(maxGameObjectCount);// [tdbe] again, this is also the size of the previous range above
    util::DebugLog("[RenderProcess][framesInFlight: " + util::ToString(Renderer::framesInFlight) +
                   "] " +
                   "Binded (bound) Descriptor Sets: total size for vertex & fragment data "
                   "(and both static and dynamic data)\n\taka uniformBufferSize: " +
                   util::ToString(uniformBufferSize) + ";\n" +
                   "\tsizeof(StaticVertexUniformData): " + util::ToString(sizeof(StaticVertexUniformData)) + ";\n" +
                   "\tsizeof(StaticFragmentUniformData): " + util::ToString(sizeof(StaticFragmentUniformData)) + ";\n" +
                   "\tsizeof(DynamicVertexUniformData): " + util::ToString(sizeof(DynamicVertexUniformData)) + ";\n" +
                   "\tsizeof(DynamicFragmentUniformData): " + util::ToString(sizeof(DynamicFragmentUniformData)) +
                   ";\n" + "\tgameObjectCount: " + util::ToString(maxGameObjectCount) + ";");
    ...
    {
        util::DebugError("[RenderProcess] can't map the uniform buffer memory!");
        valid = false;
        return;
    }
    ...
    vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 
                            0u, nullptr);

    util::DebugLog("[RenderProcess] Finished Update Render process / descriptor sets.");
}
...
void RenderProcess::ClearUniformBuffer()
{
    if (uniformBuffer)
    {
        uniformBuffer->unmap();
